app.controller('newSearchController', ['$scope', 'Factory', '$location', 'sharedProperties', 'commonFunctions', '$timeout', 'config', '$sce', '$uibModal', '$filter', '$rootScope', function ($scope, Factory, $location, sharedProperties, commonFunctions, $timeout, config, $sce, $uibModal, $filter, $rootScope) {
    ...
    $scope.alltags = [];
    $scope.tags = '';
    $scope.type = 'ns-Requisition'
    $scope.fstype = 'ns-freeSearch'
    var tagsList = {};
    ...
    $scope.fillData = function ($item, $model, $label, $event) {
        ...
        $scope.selectedReq = $filter('filter')($scope.talentReq, {
            ReqNumber: reqNum
        })[0];
        getArya(reqNum);
    }
    ...
    function callBackend(radioModel) {
        ...
        else if (tagsList.type == $scope.type) {
            data.Industries = tagsList.tags;
        }
        if (radioModel) {
            ...
            else if (tagsList.type == $scope.fstype) {
                data.Industries = tagsList.tags;
            }
        }
        /*if(data.Miles){
